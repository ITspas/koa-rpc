...
function endWithJson(statusCode, json, cxt, encoding) {
...
function error(encoding, err, cxt) {
	endWithJson(500, [serializeError(err)], cxt, encoding)
}
		...
		parseRequest(encoding, cxt.req, (err, input) => {
			...
			if (input.sync) {
				...
				input.args.push(function() {
					...
					endWithJson(200, args, cxt, encoding)
					done();
				})
